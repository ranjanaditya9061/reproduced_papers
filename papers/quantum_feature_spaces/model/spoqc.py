...
class SpoqcPhotonicTeacher(Teacher):
    ...
    def hash_spec(cls, cfg: "ExperimentConfig") -> dict:
        # spoqc-only knobs -> only this teacher's hash sees observable / cx_pairs.
        # cx_pairs is added ONLY when non-empty, so a no-entangler config keeps the
        # same hash as before this knob existed (existing datasets stay valid).
        spec = {"observable": cfg.problem.observable, "prep": "H_Rx_Ry_seeded"}
...
def render_circuit(path="spoqc.png", *, m=6, k=3, n_features=5, seed=42, cx_pairs=None, x=None):
    ...
    out = SpoqcPhotonicTeacher(m=m, k=k, n_features=n_features, seed=seed,
