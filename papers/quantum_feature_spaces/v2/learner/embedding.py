...
closed-form ridge readout.
    ...
    ridge  -- fixed/random features (fourier/raw/rff, see :mod:`.features`) + ridge
...
**Additive across coordinates, no cross terms.**  A ridge readout on any basis in
:mod:`.features` is a sum of per-coordinate functions, so it cannot represent the *product*
structure of a Fock-basis probability (a permanent/determinant is multilinear in the input
columns).  Measured: ``sweep_degree_grid`` (:mod:`.auto`) closes most of this gap by adding
``PolynomialFeatures``-expanded cross terms on top of the same Fourier base -- ``parity`` moves from
``R^2~0.07`` at ``degree=1`` to ``~0.71`` at ``order=2, degree=3``, competitive with ``svr``/``mlp``.
A previous correlation-selected adaptive-basis variant (``topk_fourier``) was tried and removed: on
every observable tested it matched plain ``fourier`` at ``order=1`` and never approached the
degree-grid's cross-term rows, so it bought nothing over the fixed grid at the same feature budget.
"""
